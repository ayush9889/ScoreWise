...
export const Dashboard: React.FC<DashboardProps> = ({ onBack }) => {
  ...
  useEffect(() => {
    ...
    const handleStatsUpdate = () => {
      console.log('Player stats updated event received, refreshing dashboard...');
      loadData();
    };
  ...
  const loadData = async () => {
    try {
      ...
      // Sort players by total performance (runs + wickets)
      groupPlayers.sort((a, b) => {
        const aScore = a.stats.runsScored + (a.stats.wicketsTaken * 20) + (a.stats.motmAwards * 50);
        const bScore = b.stats.runsScored + (b.stats.wicketsTaken * 20) + (b.stats.motmAwards * 50);
  ...
  // Enhanced leaderboard calculations with better filtering
  const topRunScorers = [...players]
    .filter(p => p.stats.matchesPlayed > 0 && p.stats.runsScored > 0)
    .sort((a, b) => b.stats.runsScored - a.stats.runsScored)
    .slice(0, 10);

  const topWicketTakers = [...players]
    .filter(p => p.stats.matchesPlayed > 0 && p.stats.wicketsTaken > 0)
    .sort((a, b) => b.stats.wicketsTaken - a.stats.wicketsTaken)
    .slice(0, 10);

  const bestAverages = [...players]
    .filter(p => p.stats.timesOut > 0 && p.stats.matchesPlayed > 0 && p.stats.runsScored >= 20)
    .sort((a, b) => parseFloat(CricketEngine.calculateBattingAverage(b.stats)) - parseFloat(CricketEngine.calculateBattingAverage(a.stats)))
    .slice(0, 10);

  const mostMOTM = [...players]
    .filter(p => p.stats.matchesPlayed > 0 && p.stats.motmAwards > 0)
                ...
                <div className="space-y-3">
                      ...
                      <div className="flex items-center">
                        ...
                        <div className="text-left">
                          ...
                          <div className="text-sm text-gray-500">
                            Avg: {CricketEngine.calculateBattingAverage(player.stats)} • SR: {CricketEngine.calculateStrikeRate(player.stats)}
                          </div>
                        </div>
                      </div>
                  ...
                  {topRunScorers.length === 0 && (
                    <div className="text-center py-8 text-gray-500">
                      ...
                      <p className="text-sm">No batting stats yet</p>
                    </div>
                  )}
                </div>
              </div>
                ...
                <div className="space-y-3">
                    ...
                    >
                      <div className="flex items-center">
                        ...
                        <div className="text-left">
                          <div className="font-medium text-gray-900">{player.name}</div>
                          <div className="text-sm text-gray-500">
                            Avg: {CricketEngine.calculateBowlingAverage(player.stats)} • Econ: {CricketEngine.calculateEconomyRate(player.stats)}
                          </div>
                        </div>
                      </div>
                      <div className="text-right">
                  ...
                  {topWicketTakers.length === 0 && (
                    <div className="text-center py-8 text-gray-500">
                      ...
                      <p className="text-sm">No bowling stats yet</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="bg-white rounded-xl shadow-sm p-6">
              ...
              {topRunScorers.length > 0 ? (
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {topRunScorers.map((player) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3 text-right font-bold">{player.stats.runsScored}</td>
                ...
                <div className="text-center py-12 text-gray-500">
                  ...
                  <p className="text-lg font-medium">No batting statistics yet</p>
                  <p className="text-sm">Complete some matches to see batting leaderboards</p>
                </div>
              )}
            </div>
          )}
            ...
            <div className="bg-white rounded-xl shadow-sm p-6">
              ...
              {topWicketTakers.length > 0 ? (
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {topWicketTakers.map((player) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3 text-right font-bold">{player.stats.wicketsTaken}</td>
                ...
                <div className="text-center py-12 text-gray-500">
                  ...
                  <p className="text-lg font-medium">No bowling statistics yet</p>
                  <p className="text-sm">Complete some matches to see bowling leaderboards</p>
                </div>
              )}
            </div>
          )}
            ...
            <div className="bg-white rounded-xl shadow-sm p-6">
              ...
              {mostMOTM.length > 0 ? (
                ...
                <div className="text-center py-12 text-gray-500">
                  ...
                  <p className="text-lg font-medium">No all-round statistics yet</p>
                  <p className="text-sm">Complete some matches to see all-round performance</p>
                </div>
              )}
            </div>
          )}
            ...
            <div className="bg-white rounded-xl shadow-sm p-6">
              ...
              {recentMatches.length > 0 ? (
                ...
                <div className="text-center py-12 text-gray-500">
                  ...
                  <p className="text-sm">Start playing matches to see match history here</p>
                </div>
              )}
            </div>
          )}
